...
| Parameter | Normal Synth. | Power-opt Synth. | Post-Route (Innovus) |
|---|---|---|---|
| Cell count | 2,240 | 2,241 | 3,085 total instances¹ |
| Cell area (µm²) | 12,160.725 | 10,005.877 | 17,369.060 (std cells) |
| Net area (µm²) | 3,003.769 | 2,231.532 | — |
| **Total area (µm²)** | **15,164.494** | **12,237.409** | 17,406.774 (core) |
...
¹ 3,085 is the total post-route instance count (functional cells + clock-tree buffers/inverters + hold-fix buffers inserted during CTS and optimization — see [the Finding in §2 of README.md](README.md#finding-which-synthesis-run-was-actually-taken-to-p-and-r)). The Innovus power report explicitly logs **0 dedicated filler/decap cells**, so the "79.796% / 99.783%-with-fillers" density pair reported by `timeDesign` reflects two different density *definitions* (physical-cell-subtracted vs. not), not a filler-insertion step.

## 3. Timing

| Metric | Genus (Normal) | Genus (Power-opt) | **Innovus (post-route)** | **Tempus (signoff)** |
|---|---|---|---|---|
| Setup WNS | +2.418 ns (example path) | +3.283 ns (`s_axi_aclk`) | **+2.633 ns** | **+2.627 ns** |
| Setup TNS | — | 0.000 | 0.000 | 0.000 |
| Hold WNS | — | — | **+0.049 ns** | **0.000 ns** |
| Hold TNS | — | — | 0.000 | 0.000 |
| Setup violating paths | — | 0 | 0 / 3,639 | 0 |
| Hold violating paths | — | — | 0 / 3,639 | 0 |
| Critical path type | — | — | I/O (`in2reg`) | I/O (`in2reg`) |
| `min_pulse_width` (endpoints) margin | — | — | — | +3.243 ns, 0 fails |

Setup/hold divergence between Innovus and Tempus: **6 ps** (setup), **49 ps** (hold) — both re-derive timing from the *same* post-route netlist + SPEF, so this is the expected engine-to-engine noise floor, not a real timing gap. Full detail: [§3 of README.md](README.md#3-comparison-2--tempus-signoff-timing-vs-innovus-implementation-timing).

## 4. Power

| Metric | Genus Normal | Genus Power-opt | **Innovus (vectorless)** | **Voltus (activity-based)** |
|---|---|---|---|---|
| Total Power | 1.800 mW | 0.370 mW | **1.871 mW** | **1.402 mW** |
| Internal Power | 1.450 mW (80.56%) | 0.255 mW (68.93%) | 1.517 mW (81.09%) | 1.163 mW (82.96%) |
| Switching Power | 0.349 mW (19.42%) | 0.114 mW (30.97%) | 0.353 mW (18.88%) | 0.238 mW (17.00%) |
| Leakage Power | 0.43 µW (0.02%) | 0.37 µW (0.10%) | **0.505 µW** | **0.505 µW** (identical — leakage is activity-independent) |
...
| Activity source | vectorless / default | vectorless / default | vectorless / default | **real SAIF from `tb_axi4_lite_coverage_verilog`** |

Full discussion: [§4 of README.md](README.md#4-comparison-3--voltus-signoff-power-vs-innovus-implementation-power).
